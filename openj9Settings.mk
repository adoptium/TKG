...
ifneq (, $(findstring current, $(JCL_VERSION)))
	ifeq (8, $(JDK_VERSION))
		ifneq (,$(findstring win,$(SPEC)))
			JAVA_SHARED_LIBRARIES_DIR_INIT:=$(TEST_JRE_BIN)$(D)$(VM_SUBDIR)
			J9VM_PATH=$(TEST_JRE_BIN)$(D)j9vm
		else
			JAVA_SHARED_LIBRARIES_DIR_INIT:=$(TEST_JRE_LIB_DIR)$(D)$(ARCH_DIR)$(D)$(VM_SUBDIR)
			J9VM_PATH=$(TEST_JRE_LIB_DIR)$(D)$(ARCH_DIR)$(D)j9vm
		endif
		LIBPATH_VAL=$(Q)$(LIBPATH)$(P)$(TEST_JRE_LIB_DIR)$(D)$(VM_SUBDIR)$(P)$(JAVA_SHARED_LIBRARIES_DIR_INIT)$(P)$(TEST_JRE_BIN)$(D)j9vm$(Q)
		CHECK_VM_SUBDIR=
		CHECK_JAVA_SHARED_LIBRARIES_DIR=
		ifeq (default, $(VM_SUBDIR))
			...
			CHECK_JAVA_SHARED_LIBRARIES_DIR=JAVA_SHARED_LIBRARIES_DIR=$(JAVA_SHARED_LIBRARIES_DIR_INIT); echo $(Q)$(JVM_OPTIONS)$(Q) | grep -q -- '-Xcompressedrefs' && JAVA_SHARED_LIBRARIES_DIR=$$(echo $(Q)$$JAVA_SHARED_LIBRARIES_DIR$(Q) | sed 's|/default|/compressedrefs|g; s|\\default|\\compressedrefs|g')
		endif
		ADD_JVM_LIB_DIR_TO_LIBPATH=$(CHECK_VM_SUBDIR) ; echo Set LIBPATH to $$LIBPATH_VAL && export LIBPATH=$$LIBPATH_VAL; 
		JAVA_SHARED_LIBRARIES_DIR=$(CHECK_JAVA_SHARED_LIBRARIES_DIR); echo Set JAVA_SHARED_LIBRARIES_DIR to $$JAVA_SHARED_LIBRARIES_DIR && export JAVA_SHARED_LIBRARIES_DIR=$$JAVA_SHARED_LIBRARIES_DIR; $$JAVA_SHARED_LIBRARIES_DIR
	else
		ifneq (,$(findstring win,$(SPEC)))
